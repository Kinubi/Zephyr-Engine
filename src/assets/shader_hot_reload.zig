...
pub const ShaderWatcher = struct {
    ...
    fn processCompilationJob(self: *Self, job: CompilationJob, thread_id: usize) !void {
        ...
        std.log.info("✅ [Thread {}] Shader compiled in {d:.2}ms: {s} ({} bytes)", .{ 
            thread_id, compile_time_ms, job.file_path, compiled_shader.spirv_code.len 
        });
    ...
    fn isShaderFile(filename: []const u8) bool {
        const extensions = &[_][]const u8{
            ...
            ".tesc", ".tese", ".hlsl", ".rchit", ".rgen", 
            ".rmiss", ".rint", ".rahit", ".rcall"
        };
