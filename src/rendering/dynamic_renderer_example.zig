...
pub const DynamicRenderer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, graphics_context: *GraphicsContext, pipeline_manager: *DynamicPipelineManager) !Self {
        var renderer = Self{
            .allocator = allocator,
    ...
    pub fn render(self: *Self, frame_info: FrameInfo, render_pass: vk.RenderPass, render_mode: RenderMode) !void {
        ...
        if (self.descriptor_set) |desc_set| {
            frame_info.command_buffer.cmdBindDescriptorSets(.graphics, pipeline_layout.?, 0, 1, @ptrCast(&desc_set), 0, null);
        }
        ...
        frame_info.command_buffer.cmdPushConstants(pipeline_layout.?, .{ .vertex_bit = true }, 0, @sizeOf(PushConstants), &push_constants);
