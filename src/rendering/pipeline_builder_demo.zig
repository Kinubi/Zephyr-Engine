...
pub const ModernGeometryPass = struct {
    ...
    pub fn execute(self: *Self, context: RenderContext) !void {
        ...
        std.log.info("  - Pipeline retrieved from cache (hit ratio: {d:.1}%)", .{
            self.pipeline_cache.getStatistics().hit_ratio * 100.0
        });
...
pub const ModernComputePass = struct {
    ...
    pub fn execute(self: *Self, context: RenderContext) !void {
        std.log.info("ModernComputePass: Executing frame {d} with pipeline cache hit rate: {d:.1}%", 
            .{context.frame_index, self.pipeline_cache.getHitRate() * 100.0});
