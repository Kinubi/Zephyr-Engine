...
pt_pass.bind("scene", tlas);                       // Acceleration structure
...
Currently `PathTracingPass` creates its own `output_texture` internally:
```zig
// CURRENT (tight coupling, manual resize/format handling):
...
// DESIRED (external creation, auto-resize + format):
...
### Phase 9: External Resource Pattern with Dynamic Resize (Next)
...
4. ⏳ Update other passes for external resources
